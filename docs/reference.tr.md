...
**Görüntüleyici bir belgeyi işler, çalıştırmaz.** Dosya sayfaya ulaşmadan önce `<script>`, `on*` işleyicileri, `javascript:` hedefleri, çerçeve ögeleri (`iframe`, `object`, `embed`) ve `<link>` çıkarılır; görüntüleyici kaçının çıkarıldığını söyler ve Kaynak görünümü her baytı olduğu gibi tutar. `<style>` çıkarılmaz, sınırlandırılır: görüntüleyiciye bağlı tek bir CSS iç içe bloğuna sarılır, böylece belge kendi biçemini korur, `body`, `html` ya da `:root` adlarını kullanan bir kural ise hiçbir şeye ulaşmaz — o üçü okurun üzerinde olduğu sayfadır ve bağlantı verilen dosyadaki `body { display: none }` eskiden o sayfayı, geri almaya yarayacak hiçbir denetim bırakmadan boşaltıyordu. Bir *sayfadaki* HTML adası bundan etkilenmez, tüm yeteneğini korur: yazar onu kendi sayfasına yazmıştır. Bağlantı verilen dosya başka bir şeydir, fark da onu kimin gözden geçirdiğidir. Şema, grafik ve canlı parçacıklar her zamanki gibi çizilir; çünkü yükleri veri niteliğindeki `text/x-mermaid`, `text/x-code` ve `application/json` taşıyıcılarında gelir. Belgesinde canlı bir ada isteyen yazar, o belgeyi bir sayfaya dönüştürür.
...
Yol önce sayfanın kendi dizinine, sonra proje köküne göre çözülür — metin, yolları kökten yazar ([`src/oku/cli.py`](#f/src/oku/cli.py)), okur da onları bir düzenleyiciye böyle yazar; sayfanın yanındaki bir dosya ise kökte aynı adlı bir dosya belirse bile anlamını korur. Mutlak yol yazıldığı gibi alınır.
...
Yolu bir kod aralığına yazacağınız her yerde buna uzanın; tablo hücreleri de buna dâhil — dosyalardan oluşan bir tablo, okurun gidip bulması gereken şeylerin listesidir ve rozet tam da bu çıkmazı kapatmak için vardır. Gerçekten var olan bir dosyayı adlandıran bir aralık gördüğünde `oku check` uyarı düzeyinde `path-in-code-span` der, `oku check --fix` ise hepsini yeniden yazar: metinde, GFM hücrelerinde ve `oku-*` yüklerinde; ön bilgiye, düz çitlere ve var olan bağlantılara dokunmadan. Yalnızca ayırıcı taşıyan aralıklar değerlendirilir, bu yüzden düz bir `kit.json` — bir cümlenin okura oluşturmasını söylediği dosya — hiçbir zaman bu projenin kendi dosyasıyla karıştırılmaz.
