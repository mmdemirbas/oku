...
Same series-of-points contract as scatter — but the renderer connects consecutive points with a polyline. Use when the x-axis is ordered (time, build-number, sequence) and the reader should see slope, not just position. Two or more series stack on the same axes; legend chips at the top-right toggle visibility. Add `"curve": "smooth"` for a Catmull-Rom spline through the points (default is `linear`); applies to `type: area` too.
...
Column variant — same flat `values` payload, set `"variant": "bar"` for per-value mini-bars. Use when each value is a discrete event (a count per day, hits per minute) rather than a continuous trend.
...
Flow between stages with curved ribbons sized by link value. `nodes[]` is `{id, label?, color?}`; `links[]` is `{source, target, value}` referencing node ids. The renderer derives the left-to-right column placement from the link graph (BFS depth), stacks nodes within each column by their incoming-or-outgoing total, and routes a cubic-Bezier ribbon between every pair.
...
Node-link diagram with a small Fruchterman-Reingold-ish force relaxation (~60 iterations on each render). Best for graphs up to ~50 nodes — bigger structures need a real graph-viz library. `nodes[]` + `links[]` share the same shape as `sankey`. Node radius scales with degree; hovering a node surfaces its label + degree in the tooltip.
...
Circular relationship diagram. `groups[]` defines the perimeter sequence (clockwise from 12 o'clock); `matrix` is an N×N flow matrix where `matrix[i][j]` is the flow from `groups[i]` to `groups[j]`. Each group occupies an outer arc sized by its total in+out flow; ribbons through the centre carry the pairwise flows.
...
N×N grid of mini scatter plots for multivariate correlation reading. `variables[]` defines each axis; `records[]` is a flat list of objects keyed by variable id. Domains auto-derive from the records unless declared explicitly.

> [!TIP] How to read it
> Diagonal cells (top-left to bottom-right) label each variable. Off-diagonal cells plot the column-variable on X against the row-variable on Y. Each cell carries an `r=` chip in its top-left — Pearson correlation between the two variables (-1 to +1). Values where `|r| ≥ 0.7` show in accent-strong: those are the strong relationships worth investigating. Hover any off-diagonal cell to see the pair, n, and r in a tooltip; click to pin.
...
One vertical axis per variable; each record draws a polyline crossing all axes at its scaled position. Best for spotting multi-variable clusters (lines that bunch together) or outliers (lines that diverge). Optional `_label` and `_color` on each record tag it visually.
...
Value per region laid out on a coarse world tile grid. NOT a real map — country outlines are not drawn — but the visual rhythm is enough for at-a-glance regional comparisons. The honest name is `tile-map`; `geo` stays as an alias.

### type: tile-map (alias: geo) {#chart-geo}

Tile cartogram. `regions[]` is a list of `{id, value, label?}` where `id` is an ISO 3166-1 Alpha-2 country code (US, GB, DE, BR, ...) or the synthetic `OTH` bucket. The kit ships a fixed tile grid (~60 cells) that approximates a flat world; absent regions render as faint placeholder outlines so the layout reads. Cell fill scales with value in a single-hue sequential ramp. This is intentionally NOT a choropleth — no country outlines, no topojson dependency.
...
## Small multiples {#family-small-multiples}

One chart shape repeated across N panels on a shared scale. The panels answer a question a single combined chart cannot: *which of these is different, and in what way* — four lines in one axis frame is a tangle, four frames side by side is a comparison.

### chart-grid {#chart-grid}

`panels[]` holds one payload per panel — the same fields the child chart would take, minus its `type`, which `child_type` sets once for the grid. `cols` fixes the column count; panels wrap. Every panel is drawn on the same y-scale, which is the whole point: a panel that looks flat IS flat relative to its neighbours.

```oku-example
{"code": {"k": "code", "src": "{\n  \"kind\": \"chart-grid\",\n  \"title\": \"Requests per second, by region\",\n  \"cols\": 4,\n  \"child_type\": \"line\",\n  \"panels\": [\n    {\n      \"title\": \"eu-west\",\n      \"series\": [\n        \"...\"\n      ]\n    },\n    {\n      \"title\": \"us-east\",\n      \"series\": [\n        \"...\"\n      ]\n    },\n    {\n      \"title\": \"ap-south\",\n      \"series\": [\n        \"...\"\n      ]\n    },\n    {\n      \"title\": \"sa-east\",\n      \"series\": [\n        \"...\"\n      ]\n    }\n  ]\n}", "lang": "json"}, "output": {"k": "chart-grid", "title": "Requests per second, by region", "cols": 4, "child_type": "line", "panels": [{"title": "eu-west", "series": [{"label": "rps", "color": "accent", "data": [{"x": 1, "y": 41}, {"x": 2, "y": 44}, {"x": 3, "y": 52}, {"x": 4, "y": 61}, {"x": 5, "y": 58}, {"x": 6, "y": 66}]}]}, {"title": "us-east", "series": [{"label": "rps", "color": "accent", "data": [{"x": 1, "y": 88}, {"x": 2, "y": 84}, {"x": 3, "y": 79}, {"x": 4, "y": 71}, {"x": 5, "y": 64}, {"x": 6, "y": 55}]}]}, {"title": "ap-south", "series": [{"label": "rps", "color": "accent", "data": [{"x": 1, "y": 12}, {"x": 2, "y": 19}, {"x": 3, "y": 31}, {"x": 4, "y": 47}, {"x": 5, "y": 66}, {"x": 6, "y": 90}]}]}, {"title": "sa-east", "series": [{"label": "rps", "color": "accent", "data": [{"x": 1, "y": 22}, {"x": 2, "y": 23}, {"x": 3, "y": 21}, {"x": 4, "y": 24}, {"x": 5, "y": 22}, {"x": 6, "y": 23}]}]}]}}
```

Read left to right: eu-west rising, us-east falling, ap-south compounding, sa-east unchanged. On one axis those four series would cross each other eleven times.
