...
> [!TLDR]
> Pages are JSON or Markdown. The browser renders them via Custom Elements; no build step needed to read. Run oku build to emit a multi-page site (with Pagefind search) or a standalone single file for offline reading.
>
> - Source: <name>.json (kit schema) or <name>.md (existing Markdown drops in unchanged).
...
Eight pages. Cards link straight to each — Reference covers core primitives; Charts / Tables / Diagrams break out the heavy ones; Glossary / Architecture / CLI go deeper.

```oku-step-flow
{"steps":[{"t":"Reference","b":"Project setup, page anatomy, Markdown sources, the prose / emphasis / structured-layout primitives, per-page metadata, build outputs.","meta":"start here · prose, layout, inline","href":"reference.html"},{"t":"Tables","b":"table primitive — flat rows, grouped rows, chip filters, board / kanban view, sticky headers, drag-resize columns, word-wrap by default.","meta":"flat · grouped · chips · board","href":"tables.html"},{"t":"Charts","b":"Every chart variant with a tiny live sample, grouped by what you're trying to show (compare · trend · distribution · composition · relationship · hierarchy · flow · location) plus rich hover, click-to-pin, pan/zoom fullscreen, and the live configure popover.","meta":"37 variants · 8 function groups","href":"charts.html"},{"t":"Diagrams","b":"diagram primitive + every Mermaid v10 type the kit forwards (flowchart, sequence, state, ER, class, gantt, pie, journey, mindmap, timeline, sankey-beta, ...). Theme tokens flow through.","meta":"Mermaid catalog","href":"diagrams.html"},{"t":"Glossary & ext-refs","b":"File layout, multi-language entries, project overrides, disambiguation syntax, resolution algorithm.","meta":"multi-domain registry","href":"glossary.html"},{"t":"Architecture","b":"How the kit fits together at runtime — render flow, tooltip controller, kit loader, build pipeline, standalone bundle. Read this when extending or debugging the kit itself.","meta":"internals · developer audience","href":"architecture.html"},{"t":"CLI reference","b":"Five commands. What each does, what output to expect, the doctree linter's severity table, the optional Pagefind and jsonschema dependencies.","meta":"init · build · clean · check · migrate · serve","href":"cli.html"},{"t":"Roadmap","b":"Ten phases, every tracked item, the principles driving the rewrite, and the open questions still on the table.","meta":"what's next · oku 1.0","href":"roadmap.html"}]}
```

## What a page looks like {#shape}

Two source formats produce the same rendered page. Pick whichever fits the content — JSON for kit-native authoring, Markdown for prose that should drop in unchanged. Code on the left, the actual rendered output on the right.
...
```oku-example
{"code":{"k":"code","src":"{\n  \"kind\": \"callout\",\n  \"type\": \"info\",\n  \"title\": \"Heads up\",\n  \"content\": \"Callouts carry one of seven tones: info, tip, note, neutral, success, caution, danger.\"\n}","lang":"json"},"output":"> [!INFO] Heads up\n> Callouts carry one of seven tones: info, tip, note, neutral, success, caution, danger."}
```

```oku-example
{"code":{"k":"code","src":"{\n  \"kind\": \"kpi-grid\",\n  \"tiles\": [\n    { \"num\": \"29\",   \"label\": \"chart variants\" },\n    { \"num\": \"JSON\", \"label\": \"single source\" },\n    { \"num\": \"0\",    \"label\": \"build steps for content\" }\n  ]\n}","lang":"json"},"output":{"k":"kpi-grid","tiles":[{"num":"29","label":"chart variants"},{"num":"JSON","label":"single source"},{"num":"0","label":"build steps for content"}]}}
```

### Markdown source {#shape-md}

Drop existing .md files into the docs root. ATX headings, code fences (mermaid included), lists, tables, blockquotes, inline formatting all convert. Relative .md links retarget to .html. README / CHANGELOG / CLAUDE.md and friends are surfaced as pages too.
...
Three commands. Adopt the kit for an existing docs/ directory without rewriting anything.
...
After `init`, any `.json` or `.md` file in the docs tree is a page. The sidebar lists them; the on-page TOC builds from H2 / H3; full-text search builds with `oku build` (if Pagefind is on PATH).
