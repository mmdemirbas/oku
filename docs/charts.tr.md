...
## Küçük çoklular {#family-small-multiples}

Tek bir grafik biçiminin, ortak ölçek üzerinde N panelde yinelenmesi. Paneller, birleşik tek bir grafiğin yanıtlayamayacağı soruyu yanıtlar: *hangisi farklı ve nasıl farklı*. Tek eksende dört çizgi bir yumaktır; yan yana dört çerçeve ise bir karşılaştırma.

### chart-grid {#chart-grid}

`panels[]` her panel için bir yük taşır — alt grafiğin alacağı alanların aynısı, yalnızca `type` hariç; onu ızgara için bir kez `child_type` belirler. `cols` sütun sayısını sabitler, paneller alt satıra taşar. Bütün paneller aynı y ölçeğinde çizilir; asıl mesele de budur: düz görünen bir panel, komşularına göre gerçekten düzdür.
...
Soldan sağa okuyun: eu-west yükseliyor, us-east düşüyor, ap-south katlanarak artıyor, sa-east değişmiyor. Tek eksende bu dört dizi birbirini on bir kez keserdi.
