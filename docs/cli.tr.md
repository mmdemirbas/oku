...
bir veri gövdesi yazdırır. Kitte 14 blok türü, 53 grafik türü ve 3 satır
içi tür var; bunların biçimi, yazarın o an yazdığı sayfaya bakarak
çıkaramayacağı tek şey.

```bash
oku spec                # bütün adlar, gruplanmış
oku spec sankey         # yapıştırmaya hazır çit
oku spec table --json   # çitsiz, yalnızca veri gövdesi
oku spec filepath       # satır içi bir tür: yazımı ve ne zaman kullanılacağı
oku spec front-matter   # sayfa düzeyindeki her anahtar ve ne işe yaradığı
oku spec kit            # kit.json'daki her anahtar ve ne işe yaradığı
```

Üç satır içi tür — [filepath](reference.tr.md#filepath),
[glossary-term](reference.tr.md#glossary-term) ve
[ext-ref](reference.tr.md#ext-ref) — çit olarak değil, cümlenin içinde
bir bağlantı olarak yazılır; burada listelenene kadar hiçbir yerde
listelenmiyorlardı. Her biri hem yazımını hem de *ne zaman* ona
uzanılacağını anlatan bir notu yazdırır: yazım "nasıl" sorusunu
karşılar, kod aralığı ile rozet arasında karar veren yazar ise "ne
zaman" diye sorar.

Bu listedeki iki ad blok türü değil. `front-matter` sayfa düzeyindeki bütün
anahtarları, `kit` ise ağacın `kit.json` dosyasındaki bütün anahtarları
yazdırır. İkisi de yanına yazılmış bir listeyi değil, belgelediği şemayı
okur; bu yüzden araçtan geri kalamazlar — nitekim `kit.json`, kitin okuduğu
ama kendi şemasının kabul etmediği beş anahtarı bu yüzden taşıyabildi.

```oku-table
{"headers":["Argüman","Varsayılan","Etkisi"],"rows":[["`name`","—","Bir blok türü (`table`, `kpi-grid`, …), bir grafik türü (`sankey`, `gantt`, …) ya da iki başlıktan biri: `front-matter` ve `kit`. Bütün adları listelemek için boş bırakın. Tanınmayan bir ad 1 ile çıkar ve en yakın eşleşmeleri önerir."],["`--json`","kapalı","Çiti değil, yalnızca veri gövdesini yazdırır. Veri gövdesini program içinde oluştururken işe yarar."]]}
```
...
Bulgular üç önem derecesine ayrılır; `--json` çıktısında bunlar `error` / `warning` / `info` olarak görünür. Herhangi bir hata varsa (ya da `--strict` altında herhangi bir uyarı varsa) çıkış kodu 1, aksi hâlde 0 olur. Bilgi notları birer öneridir, bu yüzden rapor onları yalnızca `--verbose` ile yazdırır — ama kaç tane olduğunu ve hangi kodları taşıdıklarını her zaman söyler; çünkü hiç anılmayan bir notu kimse isteyeceğini bilemez. `--fix`, uygulayabileceği bulguları uygular — bugün bu yalnızca `path-in-code-span`; metinde, GFM hücrelerinde ve `oku-*` yüklerinin içinde yeniden yazar — sonra ağacı yeniden yazılmış hâliyle denetler, böylece okuduğunuz rapor dosyalarınızın şu anki durumudur.

```oku-table
{"headers":["Önem","Kodlar","Neyi yakalar"],"rows":[["**hata**","`schema`, `deprecated-kind`, `unknown-kind`, `invalid-block`, `duplicate-anchor`, `chart-*`, `stray-demo`, `no-title`, `front-matter-malformed`, `shadowed-source`, `json-parse-failed`","Sayfa yapısı sorunları, kaldırıldığı hâlde hâlâ kullanılan yapı taşları, ne markdown metni ne de türü belirtilmiş nesne olan bir gövde girdisi, yinelenen bölüm / başlık kimlikleri, bozuk grafik yükleri, docs/reference.md dışında kalan örnek sayfalar, başlıksız bir sayfa, düzenlediğiniz kaynağı gölgeleyen bir .json. Ön bilgisi `---` ile açılıp `key: value` biçiminde olmayan bir satıra çarpan sayfa — blok, gövde metni olarak okunuyordur."],["**hata** · markdown","`fence-not-lifted`, `setext-heading`, `island-unclosed`","Gövdesi bilinen bir türden tek bir JSON nesnesi olmayan `oku-*` çiti; türlü bloğa dönüşemediği için kod bloğu olarak kalmıştır. Setext (`===` alt çizgili) başlık — katı GFM alt kümesi yalnızca ATX `#` başlıklarını kabul eder. Etiketi hiç kapanmayan bir HTML adacığı: ardından gelen her şey onun içine yazılır, yani bölümün geri kalanını da içine alır — hem sayfa sonunda hem de her `##` başlığında bildirilir, çünkü bölüm sınırı, motorun tek geçişte işlediği blok dizisini bitirir."],["**uyarı**","`process-breadcrumb`, `placeholder-text`, `unresolved-glossary`, `unresolved-extref`, `unresolved-link`, `unresolved-anchor`, `filepath-missing`, `filepath-outside`, `path-in-code-span`, `image-outside`, `unknown-meta-key`, `accent-unknown`, `translation-anchor-drift`, `kit-invalid`","Süreç ya da geçmiş göndermesi içeren metin (round-N, vN-review, fixed-in-round; kit yalnızca güncel davranışı belgeler) — paragrafta da, türü belirtilmiş yükte de, HTML adacığının içinde de; çünkü adacık, okurun içinden gördüğü bir işaretlemedir. Konusu doğrudan bir geçmiş olan sayfa — düzelttiği turu anan bir rapor, bir denetim, bir değişiklik günlüğü — ön bilgisinde `documents_history: true` bildirir ve yalnızca bu kuraldan muaf tutulur; diğer bütün denetimler ona da uygulanır. Hâlâ yer tutucu taşıyan metin (`TODO`, `TBD`, `FIXME`, `XXX`, `lorem ipsum` ya da doldurulmamış bir `{{ }}` şablonu) — paragrafta da, türü belirtilmiş yükte de, HTML adacığının içinde de; çünkü bir adım gövdesi de, bir kart da, bir `<div>`'in etiketleri arasındaki metin de yazarın yazdığı metindir. `<pre>` ya da `<script>` gövdesi değildir: orası koddur ve bir örnekteki `// TODO: implement` satırı örneğin kendisidir. Kod aralığı içindekiler geride bırakılmış değil ADI ANILAN belirteçtir, bu yüzden bildirilmez; bu satır da kendi dördünü öyle yazıyor. Projenin `kit.json` dosyasında etkinleştirdiği alanlara göre çözülemeyen sözlük terimleri ve dış kaynaklar — o alanların merkezi kayıt dosyaları artı projenin `kit.json` içindeki kendi eklemeleri; çalışma zamanının aradığı küme de budur. Projenin etkinleştirmediği bir alandaki terim ya da hiç `domains` bildirilmemişken herhangi bir kayıttaki terim, doğru yazılmış bir göndermeye yazım önerisi yerine eklenecek alanın adıyla bildirilir. HTML adacığı içindeki öğe biçimi — nitelenmiş bir göndermenin yazıldığı `<glossary-term term=\"…\" in=\"…\">` — de kendi satırında okunur; projenin ne etkinleştirdiği ne de giriş eklediği bir alanı adlandıran `in`, bulunamayan terim olarak değil, tam olarak bu durum olarak bildirilir. Sayfaya göre var olmayan bir dosyayı ya da işaret ettiği sayfada başlık kimliği bulunmayan bir `#parça` adresini gösteren bağlantı veya görsel. O yolda dosya bulunmayan ya da proje kökünün dışına çıkan bir `#f/` dosya göndermesi — sayfa, önizlediği dosyaların baytlarını içinde taşır; projenin dışına uzanan bir gönderme bunları yayımlamış olurdu, bu yüzden o rozet yalnızca kopyalanabilir bir yol gösterir. Gerçekten var olan bir dosyayı adlandıran kod aralığı — okurun sayfadan çıkıp o dosyayı araması gerekir; oysa `#f/` rozeti üstüne gelince önizler, tıklayınca açar ve yolu kopyalar. Yalnızca AYIRICI taşıyan aralıklar değerlendirilir, bu yüzden düz bir `kit.json` — bir cümlenin OKURA oluşturmasını söylediği dosya — rahat bırakılır; `oku check --fix` ise hepsini yeniden yazar. Kitin tanımadığı bir ön bilgi anahtarı; en yakın tanıdığı anahtarla birlikte bildirilir. Düz bir sözcük olan ve ne kitin ayarladığı yedi belirteçten ne de bir CSS renk adından olan bir `accent:` değeri — tarayıcı onu çözemez, sayfa da varsayılan vurgu rengini korur; yalnızca düz sözcükler değerlendirilir, çünkü onaltılık bir değeri ya da `rgb(...)` biçimini okumak tarayıcının işidir. Sayfanın bir dilinde bulunup çevirisinde bulunmayan başlık kimliği — dil düğmesi okurun parça adresini karşıya taşıdığı için, okuru içinde derinlemesine gezdiği sayfanın en başına düşürür. Proje kökünün dışındaki gerçek bir dosyaya çözümlenen görsel, `<img src>` ya da `srcset` adayı — tek dosyalık sayfa gösterdiği her şeyin baytlarını içinde taşır, yani bu gönderme projenin dışındaki bir dosyayı sayfayı alan herkese vermiş olurdu; bu yüzden iki derleme ağacı da onu taşımaz. Kitin okumadığı bir `kit.json` anahtarı — tanıdığı en yakın anahtarla birlikte bildirilir — ya da şemanın kabul etmediği biçimde bir değer. O dosyadaki her ayar arkasında bir varsayılanla okunur; bu yüzden `personalization` yerine `personalisation` yazmak hiçbir yerde hata üretmez, yalnızca ağaçtaki bütün okur yer tutucuları sessizce boş kalır — ne komut satırında, ne tarayıcı konsolunda, ne de `--strict` altında bir şey söylenir. Anahtarları `oku spec kit` yazdırır."],["**uyarı** · markdown","`ambiguous-hr`, `heading-level-skip`, `indented-code`, `lazy-continuation`, `undefined-footnote`, `undefined-link-reference`","Metin satırının hemen altındaki `---`: CommonMark'ta setext başlık, kitte yatay çizgi — öncesine boş satır koyun. Bir düzey atlayan başlık (`##` ardından doğrudan `####`); bu, ekran okuyucular ve sayfa içi içindekiler için ana hattı bozar. Boş satırdan sonra gelen dört boşluk girintili blok; CommonMark bunu kod bloğu, kit ise paragraf okur. `>` işareti olmadan sürdürülen alıntı satırı. Sayfada tanımı bulunmayan bir `[^id]` ya da `[label]` göndermesi — düz metin olarak işlenir, başka hiçbir işaret vermez."],["**uyarı** · sunum","`redundant-meta`, `prose-only-section`, `island-hand-styled`, `diagram-unknown-token`, `group-of-one`, `figure-restates-headings`","Bir başkasını yineleyen alan (`summary` metnini olduğu gibi taşıyan `subtitle`, `date` değerini taşıyan `updated`). Göze hitap eden hiçbir şey içermeyen, üç ya da daha çok paragraflık bölüm — tablo, grafik, şema, kart ızgarası ya da kod bloğu yok; bilgi kutusu sayılmaz. Kitin sınıfları ve CSS değişkenleri üzerine kurulmak yerine sabit renkler ya da kendi `<style>` bloğunu taşıyan HTML adacığı. Bu kitin tanımlamadığı bir `var(--token)` değerini adlandıran mermaid biçem satırı — kit, çözülemeyen bir belirteci yazıldığı gibi bırakır, bu yüzden Mermaid `var(` ile karşılaşır ve şeklin tamamı ayrıştırma hatası kartına dönüşür; olağan nedeni, sayfanın kurulu araçtakinden daha yeni bir kite göre yazılmış olmasıdır. Tek üyeyle kullanılmış karşılaştırma ızgarası / adım akışı / KPI ızgarası / grafik ızgarası; oysa bu yapı taşlarının bütün işi üyeler arasındaki ilişkidir. Düğüm etiketleri sayfanın kendi bölüm başlıkları olan bir şema."],["**bilgi**","`code-no-language`, `no-summary`, `html-island`, `empty-table`, `hand-set-derivable`, `accent-divergence`, `filepath-not-carried`","Dili bildirilmemiş kod blokları. meta.summary alanı olmayan sayfalar. HTML adacıkları (kitte işlenirler, dış markdown görüntüleyiciler bunları atar). Başlıkları olan ama satırı olmayan tablo. Derlemenin kendi türettiği bir alanın elle doldurulmuş olması. Aynı dizindeki üç ya da daha çok sayfanın, kit.json'da öntanımlı bir değer yokken farklı vurgu renkleri seçmesi. Sayfanın içinde taşınamayacak kadar büyük bir `#f/` dosyası; rozet dosyayı adlandırır ve yolunu kopyalar, önizleme sunmaz."]]}
```
...
[`bin/oku`](#f/bin/oku), `src/` dizinini sys.path'e ekleyip `oku.cli.main()` çağıran ince bir ara katmandır. PEP 723 satır içi üstverisi, uv ile çağrıldığında jsonschema'yı geçici bir sanal ortama çeker.
