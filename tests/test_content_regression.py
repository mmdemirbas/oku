...
class TestChromeKitMarkers:
    ...
    def test_source_stubs_are_regenerated_and_marked_generated(
        self, repo_root: Path, tmp_path: Path
    ) -> None:
        """`html-doc build` / `serve` write source-side .html stubs so
        IDE-built-in-server workflows (IntelliJ Open in Browser, VSCode
        Live Preview) resolve them without going through `html-doc serve`.

        Locks (D5 follow-up):
        - the stub carries a `<!-- Generated by html-doc ... -->` marker
          so AI agents and humans recognise it as a derived file
        - `_write_source_stubs(root)` is idempotent: re-running with no
          source changes writes zero stubs
        - hand-authored stubs (no marker) are NOT overwritten
        - cmd_build and cmd_serve both invoke `_write_source_stubs`
        """
        from html_doc import cli

        cli_src = (repo_root / "src" / "html_doc" / "cli.py").read_text(encoding="utf-8")
        assert "_GENERATED_STUB_MARKER" in cli_src, (
            "stubs must carry a marker so they're recognisable as generated"
        )
        assert "Generated by html-doc" in cli_src, (
            "marker text must say so plainly"
        )
        assert "_write_source_stubs(root)" in cli_src, (
            "cmd_build and cmd_serve must both call _write_source_stubs"
        )
        assert cli_src.count("_write_source_stubs(root)") >= 2, (
            "both cmd_build AND cmd_serve must call the writer"
        )

        # Functional test in an isolated tmp_path.
        (tmp_path / "docs").mkdir()
        (tmp_path / "docs" / "foo.json").write_text(
            json.dumps({"kind": "page", "title": "Foo", "blocks": []}),
            encoding="utf-8",
        )
        n1 = cli._write_source_stubs(tmp_path)
        assert n1 == 1, "first run should write the stub"
        stub_text = (tmp_path / "docs" / "foo.html").read_text(encoding="utf-8")
        assert cli._GENERATED_STUB_MARKER in stub_text, (
            "stub must carry the generated marker"
        )
        assert "<title>Foo</title>" in stub_text

        n2 = cli._write_source_stubs(tmp_path)
        assert n2 == 0, "second run with no changes must be a no-op"

        # Hand-authored stub (no marker) must not be overwritten.
        custom = (
            ...
            '<div class="layout"><page-nav></page-nav>'
            '<main id="main-content"></main></div></body></html>'
        )
        (tmp_path / "docs" / "foo.html").write_text(custom, encoding="utf-8")
        n3 = cli._write_source_stubs(tmp_path)
        assert n3 == 0, "hand-authored stub must not be overwritten"
        assert (tmp_path / "docs" / "foo.html").read_text(encoding="utf-8") == custom
